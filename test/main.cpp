...
void ontvang(const uint8_t *mac, const uint8_t *data, int len) {

}

void networkingBegin()
{
    WiFi.mode(WIFI_STA);
    ...
    WiFi.macAddress(esp_wifi_get_mac());

    esp_now_register_recv_cb(ontvang);
}
...
static void app_uart_read_task(void *arg)
{
    ...
    uint32_t count = 0;
    size_t size    = 0;
    uint8_t *data  = ESP_CALLOC(1, ESPNOW_DATA_LEN); espcalloc
    ...
    for (;;) {
        ...
        ret = espnow_send(ESPNOW_DATA_TYPE_DATA, BROADCAST_ADDRESS, data, size, &frame_head, portMAX_DELAY);
